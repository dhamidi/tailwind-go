...
**Namespaces without default theme values:** Some namespaces have no tokens defined in the default `theme.css`. These utilities work via bare numeric values, the spacing scale, or arbitrary values. Users may add custom theme tokens to these namespaces via `@theme`. The namespaces with no default tokens include:

- `z-index` (`--z-index-*`) — `z-50` resolves the bare integer `50`
- `opacity` (`--opacity-*`) — `opacity-50` computes `0.5` (percentage/100)
- `order` (`--order-*`) — `order-2` resolves the bare integer `2`
- `cursor` (`--cursor-*`) — cursor utilities use static keyword values only
- `columns` (`--columns-*`) — `columns-3` resolves the bare integer `3`
- `margin`, `padding`, `gap`, `inset` — resolve via the spacing scale (`--spacing`)
- `width`, `height`, `min-width`, `max-width`, `min-height`, `max-height` — resolve via the spacing scale or keywords
...
| `--default-font-family` | Default font family for text elements | `--theme(--font-sans, initial)` |
| `--default-font-feature-settings` | Default font feature settings | `--theme(--font-sans--font-feature-settings, initial)` |
| `--default-font-variation-settings` | Default font variation settings | `--theme(--font-sans--font-variation-settings, initial)` |
| `--default-mono-font-family` | Default monospace font family | `--theme(--font-mono, initial)` |
| `--default-mono-font-feature-settings` | Default monospace font feature settings | `--theme(--font-mono--font-feature-settings, initial)` |
| `--default-mono-font-variation-settings` | Default monospace font variation settings | `--theme(--font-mono--font-variation-settings, initial)` |
...
**Note:** Border, outline, ring, and inset-ring default widths (e.g., `border` → `1px`) are handled as hardcoded defaults within the static utility definitions, not via `--default-*` theme tokens.

##### Text Size and Line Height Associations

Each `--text-*` theme token has an associated `--text-*--line-height` token that defines the default line height for that font size. This dual-token pattern means that `text-sm` generates both `font-size` and `line-height` declarations:
...
When the engine resolves `text-sm`, it looks up both `--text-sm` (for `font-size`) and `--text-sm--line-height` (for `line-height`). If the `--line-height` companion token exists, the utility generates both declarations. Sizes 5xl and above default to a line height of `1` (no extra leading).

##### Color Families

The default theme defines **26 color families**, each with 11 shades (50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950). All colors use OKLCH color space values.

**Chromatic colors (17):**
red, orange, amber, yellow, lime, green, emerald, teal, cyan, sky, blue, indigo, violet, purple, fuchsia, pink, rose

**Neutral gray scales (5):**
slate, gray, zinc, neutral, stone

**Extended neutral families (4)** — tinted neutrals added in Tailwind CSS v4:
...
##### Deprecated Inline Reference Tokens

The default theme includes a backward-compatibility section with deprecated tokens. These are defined using `@theme default inline reference` and provide legacy aliases:
...
The `inline reference` modifiers mean these tokens are inlined directly into utility declarations (not emitted as CSS custom properties) and are used only as reference defaults. They enable backward-compatible shorthand utilities like `blur` (without a size suffix), `shadow` (bare), and `rounded` (bare). New code should prefer the sized variants (`blur-sm`, `shadow-sm`, `rounded-sm`).
...
## Appendix A: Default Theme Token Reference

This appendix lists all theme tokens defined in the default `theme.css`. These are the tokens available out of the box when creating a new engine via `New()`.
...
26 color families with 11 shades each (50–950), all in OKLCH color space. Plus `black` (`#000`) and `white` (`#fff`).

**Chromatic:** red, orange, amber, yellow, lime, green, emerald, teal, cyan, sky, blue, indigo, violet, purple, fuchsia, pink, rose

**Neutral:** slate, gray, zinc, neutral, stone

**Extended neutral (tinted grays):** mauve, olive, mist, taupe

Example token: `--color-blue-500: oklch(62.3% 0.214 259.815)`
...
All numeric spacing values are computed as `N * 0.25rem`. Named overrides (e.g., `--spacing-18`) take precedence over computed values.
...
### A.6 Text Sizes (with Line Heights)

| Size Token | Value | Line Height Token | Line Height Value |
|------------|-------|-------------------|-------------------|
...
| Token | Value |
|-------|-------|
| `--text-shadow-sm` | `0 1px 1px rgb(0 0 0 / 0.05)` |
| `--text-shadow` | `0 1px 3px rgb(0 0 0 / 0.1)` |
| `--text-shadow-md` | `0 2px 4px rgb(0 0 0 / 0.1)` |
| `--text-shadow-lg` | `0 4px 8px rgb(0 0 0 / 0.1)` |
...
Each animation references a `@keyframes` block also defined in the theme.
...
### A.20 Default Configuration Tokens
...
### A.21 Deprecated Reference Tokens

These are backward-compatibility tokens defined with `@theme default inline reference`:

| Token | Value | Preferred Alternative |
|-------|-------|-----------------------|
| `--blur` | `8px` | `--blur-sm` |
| `--shadow` | `0 1px 3px 0 rgb(0 0 0 / 0.1), 0 1px 2px -1px rgb(0 0 0 / 0.1)` | `--shadow-sm` |
| `--shadow-inner` | `inset 0 2px 4px 0 rgb(0 0 0 / 0.05)` | `--inset-shadow-sm` |
| `--drop-shadow` | `0 1px 2px rgb(0 0 0 / 0.1), 0 1px 1px rgb(0 0 0 / 0.06)` | `--drop-shadow-sm` |
| `--radius` | `0.25rem` | `--radius-sm` |
| `--max-width-prose` | `65ch` | (standalone) |
